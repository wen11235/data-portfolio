...
-- ---------- 0. Clean slate ----------
-- Safe to re-run: drops everything from any previous attempt first (e.g. a
-- prior run of setup.sql that got partway through before hitting a
-- privilege error). FK checks disabled just for the drop batch so table
